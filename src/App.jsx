...
export default function App() {
  ...
  function persistRecords(newRecords) {
    localStorage.setItem(storageKeyForUser(username), JSON.stringify(newRecords));
  ...
  function handleNumber(e) {
    const { name, value } = e.target;
    setForm(prev => ({ ...prev, [name]: parseInt(value || 0, 10) }));
  }
  ...
  function flatten(r, userOverride) {
    return {
      ...
      terminalPosition: r.terminalPosition,
      description: r.description,
      repairedBy: r.repairedBy,
      qty: r.qty,
      problemPhoto: r.problemPhoto ? '[image]' : '',
      drawingDetail: r.drawingDetail ? '[image]' : '',
      labelPhoto: r.labelPhoto ? '[image]' : '',
  ...
  function mergeAllAndExport(dateStr) {
    ...
    for (const k in localStorage) {
      ...
      try {
        ...
        arr.forEach(r => all.push(flatten(r, user)));
      } catch (e) {}
    }
      ...
      {!loggedIn ? (
        <div className="card">
          ...
          <div style={{ marginTop: 8 }}>
            <button className="button" onClick={saveUsername}>
              Prihlasit
            </button>
          </div>
        </div>
        ...
        <>
          <form className="card" onSubmit={saveRecord}>
            {/* Horní řádek s tlačítky – budete ho vidět hned i na iPhonu */}
            <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
              <div>
                <strong>Novy zaznam</strong>
              </div>
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button
                  type="button"
                  className="button"
                  onClick={() => setForm({ ...initialForm, date: new Date().toISOString().slice(0, 10) })}
                >
                  Novy
                </button>
              </div>
            ...
            <div className="form-grid">
              ...
              <label>
                Body no.
                <input name="bodyNo" value={form.bodyNo} onChange={handleChange} />
              </label>
              <label>
                Part no.
              ...
              <label>
                Factory
                <input name="factory" value={form.factory} onChange={handleChange} />
              </label>
              <label>
                CP
              ...
              <label style={{ gridColumn: '1 / -1' }}>
                Detail
                <textarea name="detail" value={form.detail} onChange={handleChange} rows={2}></textarea>
              </label>

              <label>
                Position name
                <input name="positionName" value={form.positionName} onChange={handleChange} />
              </label>
              <label>
                Terminal position
                <input name="terminalPosition" value={form.terminalPosition} onChange={handleChange} />
              </label>
              ...
              <label>
                Repaired by
                <input name="repairedBy" value={form.repairedBy} onChange={handleChange} />
              </label>
              <label>
                Qty
                <input name="qty" type="number" value={form.qty} onChange={handleNumber} />
              </label>
              ...
              <label style={{ gridColumn: '1 / -1' }}>
                ...
                <input
                  ...
                  capture="environment"
                  type="file"
                  onChange={e => handleImage(e, 'problemPhoto')}
                />
                {form.problemPhoto && (
                  <div style={{ marginTop: 6 }}>
                    <img src={form.problemPhoto} alt="p" className="preview-img" />
                    <button
                      type="button"
                      className="button"
                      style={{ marginTop: 4 }}
                      onClick={() => clearImage('problemPhoto')}
                    >
                      Odstranit
                    </button>
                  </div>
              ...
              <label style={{ gridColumn: '1 / -1' }}>
                ...
                <input
                  ...
                  accept="image/*"
                  type="file"
                  onChange={e => handleImage(e, 'drawingDetail')}
                />
                {form.drawingDetail && (
                  <div style={{ marginTop: 6 }}>
                    <img src={form.drawingDetail} alt="d" className="preview-img" />
                    <button
                      type="button"
                      className="button"
                      style={{ marginTop: 4 }}
                      onClick={() => clearImage('drawingDetail')}
                    >
                      Odstranit
                    </button>
                  </div>
              ...
              <label style={{ gridColumn: '1 / -1' }}>
                Label photo
                <input
                  ref={el => (fileRefs.current['l'] = el)}
                  accept="image/*"
                  type="file"
                  onChange={e => handleImage(e, 'labelPhoto')}
                />
                {form.labelPhoto && (
                  <div style={{ marginTop: 6 }}>
                    <img src={form.labelPhoto} alt="l" className="preview-img" />
                    <button
                      type="button"
                      className="button"
                      style={{ marginTop: 4 }}
                      onClick={() => clearImage('labelPhoto')}
                    >
                      Odstranit
                    </button>
                  </div>
            ...
            {/* spodní tlačítka (můžeš je ignorovat, ale někdy jsou praktická) */}
            <div style={{ marginTop: 10 }}>
              ...
              <button
                type="button"
                className="button"
                onClick={() => setForm({ ...initialForm, date: form.date })}
                style={{ marginLeft: 8 }}
              >
                Vymazat pole
              </button>
          ...
          <div className="card" style={{ marginTop: 12 }}>
            ...
            <div>
              {records.length === 0 && (
                <div style={{ color: '#6b7280' }}>Zadne zaznamy</div>
              )}
              {records.map(r => (
                  ...
                  <div style={{ display: 'flex', justifyContent: 'space-between' }}>
                    <div>
                      <div style={{ fontSize: 13, color: '#6b7280' }}>
                        {r.date} • {r.model} • {r.partNo}
                      </div>
                      <div style={{ fontWeight: 600 }}>{r.problem || '—'}</div>
                    </div>
                    <div>
                      <button className="button" onClick={() => editRecord(r.id)}>
                        Edit
                      </button>
                      <button
                        className="button"
            ...
            <div style={{ marginTop: 10 }}>
              ...
              <div style={{ marginTop: 8 }}>
                <button
                  className="button"
                  onClick={() =>
                    exportForDate(document.getElementById('exportDate').value)
                  }
                >
                  Exportovat (Excel)
                </button>
